...
void VGMapLoader::loadMap(string filename) {
	...
	*/
	...
	this->map = vector<vector<BuildingTile*>>((temp_map[0].size()), vector<BuildingTile*>(temp_map.size()));
		...
		for (size_t x = 0; x < map.size(); x++)
			if (*map[x][y]->valid) {
				if (y != 0)
					...
					map[x][y]->edge_list[0] = new BuildingTile(x, y - 1, false); // give dummy invalid node

				if (x != map.size() - 1)
					map[x][y]->edge_list[1] = map[x + 1][y]; // East
				else
					map[x][y]->edge_list[1] = new BuildingTile(x + 1, y, false);

				if (y != map[x].size() - 1)
					map[x][y]->edge_list[2] = map[x][y + 1]; // South
				else
					map[x][y]->edge_list[2] = new BuildingTile(x, y + 1, false);

				if (x != 0)
					map[x][y]->edge_list[3] = map[x - 1][y]; // West
				else
					map[x][y]->edge_list[3] = new BuildingTile(x - 1, y, false);
			}
